...
export default function DhUserGamePage() {
  ...
  const [cumPnL, setCumPnL] = useState({ dh: 0, gh: 0, user_a: 0, user_z: 0 });
  const [carryPnL, setCarryPnL] = useState({ dh: 0, gh: 0 });
  ...
  const restoreGame = async (gid) => {
    try {
      const res = await apiCaller.get(DH_GAMES_API.STATE(gid) + "?mode=user");
      const data = res.data;
      setGameId(data.game_id);
      setResults(data.results || []);
      setCumPnL(data.cum_pnl || { dh: 0, gh: 0, user_a: 0, user_z: 0 });
  ...
  useEffect(() => {
    ...
    if (isNew) {
      ...
      let cancelled = false;
      apiCaller.get(DH_GAMES_API.LAST_ACTIVE + "?mode=user").then(async (res) => {
        if (cancelled) return;
  ...
  const handleDeleteOne = useCallback(async () => {
    ...
    try {
      const res = isLinked
        ? await apiCaller.delete(LINKED_GAMES_API.LAST_ROUND, { game_type: "dh", game_id: gameId })
  ...
  return (
    <Box sx={{ p: isMobile ? 0.5 : 2 }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: isMobile ? 0.4 : 0.5, mb: 1, flexWrap: "wrap" }}>
        ...
        <Box sx={{ width: isMobile ? 8 : 12 }} />
        {/* 셋업 (유저 설정) */}
        <Box onClick={() => navigate(`/dhgame/user-setup${gameId ? `?gameId=${gameId}` : ""}`)} sx={{ ...controlBtnSx, cursor: "pointer", border: "2px solid #ff9800" }}>
        ...
        return (
          ...
          <table style={{ borderCollapse: "collapse", width: "fit-content", marginBottom: 12, filter: anyBet ? "none" : "grayscale(100%)", opacity: anyBet ? 1 : 0.8 }}>
            <tbody>
              {/* 0행: DH 단계 */}
              <tr>
                <td style={{ ...dcB, color: "#ff9800" }}>DH단계</td>
                <td style={{ ...dcB, color: (dh?.step || 0) > 1 ? "#f44336" : "#fff" }}>{`M${dh?.step || 0}`}</td>
