...
export default function DhGamePage() {
  ...
  const [dhGrids, setDhGrids] = useState(null);
  const [cumPnL, setCumPnL] = useState({ dh: 0, gh: 0, pinch: 0 });
  const [carryPnL, setCarryPnL] = useState({ dh: 0, gh: 0, pinch: 0 });
  ...
  const restoreGame = async (gid) => {
    try {
      const res = await apiCaller.get(DH_GAMES_API.STATE(gid));
      const data = res.data;
      setGameId(data.game_id);
      setResults(data.results || []);
      setCumPnL(data.cum_pnl || { dh: 0, gh: 0, pinch: 0 });
    ...
    if (isNew) {
      const doNew = async () => {
        if (gameId && results.length > 0) {
          try { await apiCaller.post(DH_GAMES_API.END, null, { params: { game_id: gameId } }); } catch {}
        }
        setEndingMode(false); setEndingSnapshot(null); setEndingDone(false);
        setResults([]); setPickHistory([]); setCumPnL({ dh: 0, gh: 0, pinch: 0 }); setCarryPnL({ dh: 0, gh: 0, pinch: 0 });
      ...
      let cancelled = false;
      apiCaller.get(DH_GAMES_API.LAST_ACTIVE + "?mode=admin").then(async (res) => {
        if (cancelled) return;
        const game = res.data?.game;
        if (game && game.round_count > 0) {
          setResumeGame(game);
        } else {
          if (game) { try { await apiCaller.post(DH_GAMES_API.END, null, { params: { game_id: game.game_id } }); } catch {} }
  ...
  const handleInput = async (inputValue) => {
    ...
    try {
      ...
      setCumPnL({ dh: data.cum_pnl.dh, gh: data.cum_pnl.gh, pinch: data.cum_pnl.pinch });
      const { globalhit, bet, dh_grids: grids, ...pick } = data;
      setPickResult({ method: pick.method, pick: pick.pick, match_start: pick.match_start, match_end: pick.match_end, matches: pick.matches, order: pick.order, prev_picks: pick.prev_picks, nickname: pick.nickname, code1: pick.code1, code2: pick.code2 });
      if (grids) setDhGrids(grids);
      setGlobalhitData(globalhit || []);
      setBetData(bet || null);
  ...
  const handleDeleteOne = useCallback(async () => {
    ...
    try {
      ...
      setCumPnL({ dh: data.cum_pnl.dh, gh: data.cum_pnl.gh, pinch: data.cum_pnl.pinch });
      const { globalhit, bet, status, ending_snapshot: snap, dh_grids: grids, ...pick } = data;
      setPickResult({ method: pick.method, pick: pick.pick, match_start: pick.match_start, match_end: pick.match_end, matches: pick.matches, order: pick.order, prev_picks: pick.prev_picks, nickname: pick.nickname, code1: pick.code1, code2: pick.code2 });
      if (grids) setDhGrids(grids);
      setGlobalhitData(globalhit || []);
      setBetData(bet || null);
  ...
  const handleEndingMode = async () => {
    ...
    if (!snapshot.nc && snapshot.gh.size === 0 && snapshot.pinch.size === 0) {
      try {
        await apiCaller.post(DH_GAMES_API.ENDING, { game_id: gameId, snapshot: { nc: false, gh: [], pinch: [] } });
    ...
    try {
      const res = await apiCaller.post(DH_GAMES_API.ENDING, {
        game_id: gameId,
        snapshot: { nc: snapshot.nc, gh: [...snapshot.gh], pinch: [...snapshot.pinch] },
      });
      const data = res.data;
      const { globalhit, bet, game_id: _gid, status: _st, snapshot: _snap, dh_grids: grids, ...pick } = data;
      setPickResult({ method: pick.method, pick: pick.pick, match_start: pick.match_start, match_end: pick.match_end, matches: pick.matches, order: pick.order, prev_picks: pick.prev_picks, nickname: pick.nickname, code1: pick.code1, code2: pick.code2 });
      if (grids) setDhGrids(grids);
      setGlobalhitData(globalhit || []);
      setBetData(bet || null);
  ...
  const handleFinishGame = async () => {
    if (gameId) { try { await apiCaller.post(DH_GAMES_API.END, null, { params: { game_id: gameId } }); } catch {} }
    setEndingMode(false); setEndingSnapshot(null); setEndingDone(false);
    setResults([]); setPickHistory([]); setCumPnL({ dh: 0, gh: 0, pinch: 0 }); setCarryPnL({ dh: 0, gh: 0, pinch: 0 });
    setBetData(null); setPickResult({ method: "wait", pick: null }); setGlobalhitData([]);
  ...
  const handleNextGame = async () => {
    ...
    try {
      ...
      else { setCumPnL({ dh: 0, gh: 0, pinch: 0 }); setCarryPnL({ dh: 0, gh: 0, pinch: 0 }); }
  ...
  return (
    <Box sx={{ p: isMobile ? 0.5 : 2 }}>
      <Box sx={{ mb: 1, display: "flex", alignItems: "baseline", gap: 1 }}>
        <span style={{ fontSize: 14, fontWeight: "bold", color: "#fff" }}>드림히트</span>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mb: 1, flexWrap: "wrap" }}>
        ...
        {/* DH step */}
        {(() => {
          const dhStep = betData?.dreamhit?.step || 0;
          return (
            <Box sx={{ ...toggleBtnSx, border: "2px solid #4caf50" }}>
              <Typography variant="caption" sx={{ fontSize: isMobile ? 9 : 11, fontWeight: "bold", color: "#fff" }}>M{dhStep}</Typography>
            </Box>
          );
        })()}
        {/* global */}
        ...
        {/* set-up */}
        <Box onClick={() => navigate(`/dhgame/setup${gameId ? `?gameId=${gameId}` : ""}`)} sx={{ ...controlBtnSx, cursor: "pointer", border: "2px solid rgba(255,255,255,0.3)" }}>
      ...
      {(() => {
        ...
        return (
          ...
          <Box sx={{ display: "flex", alignItems: "stretch", gap: 0.5, mb: 0.5, flexWrap: "wrap" }}>
            ...
            {(() => {
              const dhHasBet = (betData?.dreamhit?.amount || 0) > 0;
              const ghHasBet = (betData?.globalhit?.P || 0) + (betData?.globalhit?.B || 0) > 0;
              const pinchOn = betData?.pinch?.active || false;
              const items = [
                { name: "DreamHit", pnl: cumPnL.dh || 0, active: dhHasBet },
            ...
            {(() => {
              const totalPnL = (cumPnL.dh || 0) + (cumPnL.gh || 0);
          ...
          {(() => {
            const anyBet = (dh?.amount || 0) > 0 || (gh?.P || 0) + (gh?.B || 0) > 0;
            return (
          <table style={{ borderCollapse: "collapse", width: "fit-content", marginBottom: 12, filter: anyBet ? "none" : "grayscale(100%)", opacity: anyBet ? 1 : 0.8 }}>
            <tbody>
              {/* 1행: 섹션 상세 */}
              {(() => {
                const dhHasBet = (dh?.amount || 0) > 0;
                const ghHasBet = (gh?.P || 0) + (gh?.B || 0) > 0;
                const dhDimStyle = dhHasBet ? {} : { filter: "grayscale(100%)", opacity: 0.7 };
                const ghDimStyle = ghHasBet ? {} : { filter: "grayscale(100%)", opacity: 0.7 };
                return (
                  <tr>
                    <td style={{ ...dcB, color: "#00bcd4", ...dhDimStyle }}>DreamHit</td>
                    <td style={{ ...dc, color: "#1565c0", ...dhDimStyle }}>{`${(dh?.direction === "P" ? (dh?.amount || 0) : 0).toLocaleString()}P`}</td>
                    <td style={{ ...dc, color: "#f44336", ...dhDimStyle }}>{`${(dh?.direction === "B" ? (dh?.amount || 0) : 0).toLocaleString()}P`}</td>
                    ...
                    <td style={{ ...dc, color: "#1565c0" }}>{`${((dh?.direction === "P" ? (dh?.amount || 0) : 0) + (gh?.P || 0)).toLocaleString()}P`}</td>
                    <td style={{ ...dc, color: "#f44336" }}>{`${((dh?.direction === "B" ? (dh?.amount || 0) : 0) + (gh?.B || 0)).toLocaleString()}P`}</td>
                  </tr>
                );
              })()}
          ...
          {(() => {
            ...
            const formalData = dhGrids?.formal || [];
            const reverseData = dhGrids?.reverse || [];
                    ...
                    {label === "formal" && (
                      <Box onClick={() => {
                        ...
                        const doStart = async () => {
                          ...
                          setEndingMode(false); setEndingSnapshot(null); setEndingDone(false);
                          setResults([]); setCumPnL({ dh: 0, gh: 0 }); setCarryPnL({ dh: 0, gh: 0 });
                          setBetData(null); setPickResult({ method: "wait", pick: null }); setGlobalhitData([]);
      ...
      <Dialog open={endingDone} onClose={() => {}}>
        ...
        <DialogContent>
          ...
          <Box sx={{ mt: 2 }}>
            ...
            <Typography sx={{ mt: 1, fontWeight: "bold", color: ((cumPnL.dh || 0) + (cumPnL.gh || 0)) >= 0 ? "#4caf50" : "#f44336" }}>
              Total: {((cumPnL.dh || 0) + (cumPnL.gh || 0)) > 0 ? "+" : ""}{((cumPnL.dh || 0) + (cumPnL.gh || 0)).toLocaleString()}P
            </Typography>
          </Box>
        </DialogContent>
        <DialogActions>
      ...
      {/* 이전 게임 복원 확인 */}
      <Dialog open={!!resumeGame} onClose={async () => { const gid = resumeGame?.game_id; setResumeGame(null); if (gid) { try { await apiCaller.post(DH_GAMES_API.END, null, { params: { game_id: gid } }); } catch {} } startGame(); }}>
        ...
        <DialogActions>
          <Button onClick={async () => { const gid = resumeGame.game_id; setResumeGame(null); try { await apiCaller.post(DH_GAMES_API.END, null, { params: { game_id: gid } }); } catch {} startGame(); }}>새 게임</Button>
