...
import { DH_SETTINGS_API } from "@/constants/api-url";
...
export default function SetupPage() {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const res = await apiCaller.get(DH_SETTINGS_API.BASE);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await apiCaller.put(DH_SETTINGS_API.BASE, { config });
      setConfig(res.data.config);
      setIsLocked(res.data.is_locked);
      setIsPaused(res.data.is_paused);
      setDirty(false);
  ...
  const handlePauseResume = async () => {
    try {
      const url = isPaused ? DH_SETTINGS_API.GAME_RESUME : DH_SETTINGS_API.GAME_PAUSE;
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2 }}>
        <Box
          onClick={() => { const gid = searchParams.get("gameId"); navigate(gid ? `/dhgame?gameId=${gid}` : "/dhgame"); }}
          sx={{
            display: "inline-flex",
            alignItems: "center",
            border: "1px solid rgba(255,255,255,0.3)",
