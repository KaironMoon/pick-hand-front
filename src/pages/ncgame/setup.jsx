...
import { NC_SETTINGS_API } from "@/constants/api-url";
  ...
  return (
    <table style={{ borderCollapse: "collapse", width: "100%", tableLayout: "fixed", marginBottom: 16 }}>
      ...
      <tbody>
        ...
        {/* NC 고유: SR매칭 + 목표 + ED 한 줄 */}
        <tr>
          <td style={normalCell}></td>
          <td style={normalCell}>1~3회SR매칭</td>
          <td style={normalCell}>4~6회SR매칭</td>
...
export default function SetupPage() {
  ...
  const fetchSettings = useCallback(async () => {
    try {
      const res = await apiCaller.get(NC_SETTINGS_API.BASE);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await apiCaller.put(NC_SETTINGS_API.BASE, { config });
      setConfig(res.data.config);
      setIsLocked(res.data.is_locked);
      setIsPaused(res.data.is_paused);
      setDirty(false);
  ...
  const handlePauseResume = async () => {
    try {
      const url = isPaused ? NC_SETTINGS_API.GAME_RESUME : NC_SETTINGS_API.GAME_PAUSE;
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2 }}>
        <Box
          onClick={() => { const gid = searchParams.get("gameId"); navigate(gid ? `/ncgame?gameId=${gid}` : "/ncgame"); }}
          sx={{
            display: "inline-flex",
            alignItems: "center",
            border: "1px solid rgba(255,255,255,0.3)",
