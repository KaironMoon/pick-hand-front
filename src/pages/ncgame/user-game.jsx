...
import { NC_GAMES_API, LINKED_GAMES_API, USER_BET_SETTINGS_API } from "@/constants/api-url";
...
export default function NcUserGamePage() {
  ...
  const [cumPnL, setCumPnL] = useState({ nc: 0, gh: 0, user_a: 0, user_z: 0 });
  const [carryPnL, setCarryPnL] = useState({ nc: 0, gh: 0 });
  ...
  const handleLinkedUpdate = useCallback(() => {
    if (gameId) {
      apiCaller.get(NC_GAMES_API.STATE(gameId) + "?mode=user").then((res) => {
        const data = res.data;
        setResults(data.results || []);
        setCumPnL(data.cum_pnl || { nc: 0, gh: 0, user_a: 0, user_z: 0 });
        setPickResult({ method: data.method, pick: data.pick });
        if (data.nc_grids) setNcGrids(data.nc_grids);
  ...
  const { isLinked, linkedRound, linkedNext, linkedEnd, linkedDeleteLastRound } = useLinkedGame("nc", gameId, results.length, handleLinkedUpdate);
  ...
  const applyResponseData = (data) => {
    if (data.method !== undefined) setPickResult({ method: data.method, pick: data.pick || null });
    if (data.nc_grids) setNcGrids(data.nc_grids);
    setGlobalhitData(data.globalhit || []);
    setBetData(data.bet || null);
  ...
  const resetAll = () => {
    ...
    setResults([]); setCumPnL({ nc: 0, gh: 0, user_a: 0, user_z: 0 }); setCarryPnL({ nc: 0, gh: 0 });
  ...
  const startGame = useCallback(async (refSeq) => {
    try {
      ...
      let res;
      if (useLinked) {
        res = await apiCaller.post(LINKED_GAMES_API.START, { game_type: "nc" });
      } else {
        const params = new URLSearchParams({ mode: "user" });
        if (refSeq) params.set("ref_game_seq", refSeq);
        res = await apiCaller.post(NC_GAMES_API.START + "?" + params.toString());
      }
      const data = res.data;
  ...
  const restoreGame = async (gid) => {
    try {
      const res = await apiCaller.get(NC_GAMES_API.STATE(gid) + "?mode=user");
      const data = res.data;
      setGameId(data.game_id);
      setResults(data.results || []);
      setCumPnL(data.cum_pnl || { nc: 0, gh: 0, user_a: 0, user_z: 0 });
  ...
  useEffect(() => {
    ...
    if (isNew) {
      ...
      let cancelled = false;
      apiCaller.get(NC_GAMES_API.LAST_ACTIVE + "?mode=user").then(async (res) => {
        if (cancelled) return;
  ...
  const handleInput = async (inputValue) => {
    ...
    try {
      const res = isLinked
        ? await apiCaller.post(LINKED_GAMES_API.ROUND, { game_type: "nc", game_id: gameId, actual: inputValue })
        : await apiCaller.post(NC_GAMES_API.ROUND, { game_id: gameId, actual: inputValue });
      const data = res.data;
      ...
      setCumPnL({ nc: data.cum_pnl.nc, gh: data.cum_pnl.gh, user_a: data.cum_pnl.user_a || 0, user_z: data.cum_pnl.user_z || 0 });
      setPickResult({ method: data.method, pick: data.pick });
      if (data.nc_grids) setNcGrids(data.nc_grids);
      setGlobalhitData(data.globalhit || []);
      setBetData(data.bet || null);
  ...
  const handleNextGame = async () => {
    ...
    try {
      const res = isLinked
        ? await apiCaller.post(LINKED_GAMES_API.NEXT, { game_type: "nc", game_id: gameId })
        : await apiCaller.post(NC_GAMES_API.NEXT, null, { params: { game_id: gameId } });
  ...
  return (
    <Box sx={{ p: isMobile ? 0.5 : 2 }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: isMobile ? 0.4 : 0.5, mb: 1, flexWrap: "wrap" }}>
        ...
        <Box sx={{ width: isMobile ? 8 : 12 }} />
        {/* 셋업 (유저 설정) */}
        <Box onClick={() => navigate(`/ncgame/user-setup${gameId ? `?gameId=${gameId}` : ""}`)} sx={{ ...controlBtnSx, cursor: "pointer", border: "2px solid #ff9800" }}>
