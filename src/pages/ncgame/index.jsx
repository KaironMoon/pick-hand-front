...
import { NC_GAMES_API } from "@/constants/api-url";
...
export default function NcGamePage() {
  ...
  const [ncGrids, setNcGrids] = useState(null);
  const [cumPnL, setCumPnL] = useState({ nc: 0, gh: 0, pinch: 0 });
  const [carryPnL, setCarryPnL] = useState({ nc: 0, gh: 0, pinch: 0 });
  ...
  const startGame = useCallback(async (refSeq) => {
    try {
      ...
      const res = await apiCaller.post(NC_GAMES_API.START + "?" + params.toString());
      const data = res.data;
  ...
  const restoreGame = async (gid) => {
    try {
      const res = await apiCaller.get(NC_GAMES_API.STATE(gid));
      const data = res.data;
      setGameId(data.game_id);
      setResults(data.results || []);
      setCumPnL(data.cum_pnl || { nc: 0, gh: 0, pinch: 0 });
    ...
    if (isNew) {
      const doNew = async () => {
        if (gameId && results.length > 0) {
          try { await apiCaller.post(NC_GAMES_API.END, null, { params: { game_id: gameId } }); } catch {}
        }
        setEndingMode(false); setEndingSnapshot(null); setEndingDone(false);
        setResults([]); setPickHistory([]); setCumPnL({ nc: 0, gh: 0, pinch: 0 }); setCarryPnL({ nc: 0, gh: 0, pinch: 0 });
      ...
      let cancelled = false;
      apiCaller.get(NC_GAMES_API.LAST_ACTIVE + "?mode=admin").then(async (res) => {
        if (cancelled) return;
        const game = res.data?.game;
        if (game && game.round_count > 0) {
          setResumeGame(game);
        } else {
          if (game) { try { await apiCaller.post(NC_GAMES_API.END, null, { params: { game_id: game.game_id } }); } catch {} }
  ...
  const handleInput = async (inputValue) => {
    ...
    setPickHistory((prev) => [...prev, {
      ...
      step: betData?.nicechoice?.step || 0,
    }]);

    try {
      const res = await apiCaller.post(NC_GAMES_API.ROUND, { game_id: gameId, actual: inputValue });
      const data = res.data;
      ...
      setCumPnL({ nc: data.cum_pnl.nc, gh: data.cum_pnl.gh, pinch: data.cum_pnl.pinch });
      const { globalhit, bet, nc_grids: grids, ...pick } = data;
      setPickResult({ method: pick.method, pick: pick.pick, match_start: pick.match_start, match_end: pick.match_end, matches: pick.matches, order: pick.order });
      if (grids) setNcGrids(grids);
      setGlobalhitData(globalhit || []);
      setBetData(bet || null);
  ...
  const handleDeleteOne = useCallback(async () => {
    ...
    setProcessing(true);
    try {
      const res = await apiCaller.delete(NC_GAMES_API.LAST_ROUND(gameId));
      const data = res.data;
      setResults(results.slice(0, -1));
      setPickHistory((prev) => prev.slice(0, -1));
      setCumPnL({ nc: data.cum_pnl.nc, gh: data.cum_pnl.gh, pinch: data.cum_pnl.pinch });
      const { globalhit, bet, status, ending_snapshot: snap, nc_grids: grids, ...pick } = data;
      setPickResult({ method: pick.method, pick: pick.pick, match_start: pick.match_start, match_end: pick.match_end, matches: pick.matches, order: pick.order });
      if (grids) setNcGrids(grids);
      setGlobalhitData(globalhit || []);
      setBetData(bet || null);
  ...
  const handleEndingMode = async () => {
    ...
    if (!snapshot.nc && snapshot.gh.size === 0 && snapshot.pinch.size === 0) {
      try {
        await apiCaller.post(NC_GAMES_API.ENDING, { game_id: gameId, snapshot: { nc: false, gh: [], pinch: [] } });
    ...
    try {
      const res = await apiCaller.post(NC_GAMES_API.ENDING, {
        game_id: gameId,
        snapshot: { nc: snapshot.nc, gh: [...snapshot.gh], pinch: [...snapshot.pinch] },
      });
      const data = res.data;
      const { globalhit, bet, game_id: _gid, status: _st, snapshot: _snap, nc_grids: grids, ...pick } = data;
      setPickResult({ method: pick.method, pick: pick.pick, match_start: pick.match_start, match_end: pick.match_end, matches: pick.matches, order: pick.order });
      if (grids) setNcGrids(grids);
      setGlobalhitData(globalhit || []);
      setBetData(bet || null);
  ...
  const handleFinishGame = async () => {
    if (gameId) { try { await apiCaller.post(NC_GAMES_API.END, null, { params: { game_id: gameId } }); } catch {} }
    setEndingMode(false); setEndingSnapshot(null); setEndingDone(false);
    setResults([]); setPickHistory([]); setCumPnL({ nc: 0, gh: 0, pinch: 0 }); setCarryPnL({ nc: 0, gh: 0, pinch: 0 });
    setBetData(null); setPickResult({ method: "wait", pick: null }); setGlobalhitData([]);
  ...
  const handleNextGame = async () => {
    ...
    setProcessing(true);
    try {
      const res = await apiCaller.post(NC_GAMES_API.NEXT, null, { params: { game_id: gameId } });
      ...
      else { setCumPnL({ nc: 0, gh: 0, pinch: 0 }); setCarryPnL({ nc: 0, gh: 0, pinch: 0 }); }
  ...
  return (
    <Box sx={{ p: isMobile ? 0.5 : 2 }}>
      <Box sx={{ mb: 1, display: "flex", alignItems: "baseline", gap: 1 }}>
        <span style={{ fontSize: 14, fontWeight: "bold", color: "#fff" }}>나이스초이스</span>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, mb: 1, flexWrap: "wrap" }}>
        ...
        {/* NC step */}
        {(() => {
          const ncStep = betData?.nicechoice?.step || 0;
          return (
            <Box sx={{ ...toggleBtnSx, border: "2px solid #4caf50" }}>
              <Typography variant="caption" sx={{ fontSize: isMobile ? 9 : 11, fontWeight: "bold", color: "#fff" }}>M{ncStep}</Typography>
            </Box>
          );
        })()}
        {/* global */}
        ...
        {/* set-up */}
        <Box onClick={() => navigate(`/ncgame/setup${gameId ? `?gameId=${gameId}` : ""}`)} sx={{ ...controlBtnSx, cursor: "pointer", border: "2px solid rgba(255,255,255,0.3)" }}>
      ...
      {(() => {
        ...
        return (
          ...
          <Box sx={{ display: "flex", alignItems: "stretch", gap: 0.5, mb: 0.5, flexWrap: "wrap" }}>
            ...
            {(() => {
              const ncHasBet = (betData?.nicechoice?.amount || 0) > 0;
              const ghHasBet = (betData?.globalhit?.P || 0) + (betData?.globalhit?.B || 0) > 0;
              const pinchOn = betData?.pinch?.active || false;
              const items = [
                { name: "NiceChoice", pnl: cumPnL.nc || 0, active: ncHasBet },
            ...
            {(() => {
              const totalPnL = (cumPnL.nc || 0) + (cumPnL.gh || 0);
          ...
          <table style={{ borderCollapse: "collapse", width: "fit-content", marginBottom: 12, filter: anyBet ? "none" : "grayscale(100%)", opacity: anyBet ? 1 : 0.8 }}>
            <tbody>
              ...
              {(() => {
                ...
                return (
                  <tr>
                    <td style={{ ...dcB, color: "#00bcd4", ...ncDimStyle }}>NiceChoice</td>
          ...
          {(() => {
            ...
            const formalData = ncGrids?.formal || [];
            const reverseData = ncGrids?.reverse || [];
                    ...
                    {label === "formal" && (
                      <Box onClick={() => {
                        ...
                        const doStart = async () => {
                          ...
                          setEndingMode(false); setEndingSnapshot(null); setEndingDone(false);
                          setResults([]); setCumPnL({ nc: 0, gh: 0 }); setCarryPnL({ nc: 0, gh: 0 });
                          setBetData(null); setPickResult({ method: "wait", pick: null }); setGlobalhitData([]);
            ...
            return (
              <Box sx={{ mt: 1 }}>
                ...
                {renderGrid("reverse", reverseData, "#1565c0")}
              </Box>
            );
          })()}
      ...
      <Dialog open={endingDone} onClose={() => {}}>
        ...
        <DialogContent>
          ...
          <Box sx={{ mt: 2 }}>
            {[
              { name: "NiceChoice", pnl: cumPnL.nc || 0 },
            ...
            <Typography sx={{ mt: 1, fontWeight: "bold", color: ((cumPnL.nc || 0) + (cumPnL.gh || 0)) >= 0 ? "#4caf50" : "#f44336" }}>
              Total: {((cumPnL.nc || 0) + (cumPnL.gh || 0)) > 0 ? "+" : ""}{((cumPnL.nc || 0) + (cumPnL.gh || 0)).toLocaleString()}P
            </Typography>
          </Box>
        </DialogContent>
        <DialogActions>
      ...
      {/* 이전 게임 복원 확인 */}
      <Dialog open={!!resumeGame} onClose={async () => { const gid = resumeGame?.game_id; setResumeGame(null); if (gid) { try { await apiCaller.post(NC_GAMES_API.END, null, { params: { game_id: gid } }); } catch {} } startGame(); }}>
        ...
        <DialogActions>
          <Button onClick={async () => { const gid = resumeGame.game_id; setResumeGame(null); try { await apiCaller.post(NC_GAMES_API.END, null, { params: { game_id: gid } }); } catch {} startGame(); }}>새 게임</Button>
