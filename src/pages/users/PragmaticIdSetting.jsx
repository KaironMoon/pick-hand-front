/**
 * 본인 pickhand_id 등록/해제 — mvp-aboo-integration.md §4.5.
 *
...
const PragmaticIdSetting = ({ currentValue = "", onUpdated }) => {
  ...
  const handleSave = async () => {
    ...
    setBusy(true);
    try {
      const resp = await autoService.updatePickhandId(value.trim() || null);
      setStatus({ ok: true, msg: resp.pickhand_id ? `등록됨: ${resp.pickhand_id}` : "해제됨" });
      onUpdated?.(resp.pickhand_id);
    } catch (e) {
      const detail = e?.response?.data?.detail;
      if (e?.response?.status === 503) {
        setStatus({ ok: false, msg: "자동 베팅 기능 비활성화" });
      } else if (e?.response?.status === 409) {
        setStatus({ ok: false, msg: "이미 사용 중인 pickhand_id" });
      } else {
  ...
  return (
    <Box sx={{ maxWidth: 480, p: 2 }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        자동 베팅 사용 시 모바일 앱이 같은 pickhand_id로 JSESSIONID를 보냅니다.
      </Typography>
