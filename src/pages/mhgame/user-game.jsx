...
import { MH_GAMES_API, LINKED_GAMES_API, USER_BET_SETTINGS_API } from "@/constants/api-url";
...
export default function MhUserGamePage() {
  ...
  const handleLinkedUpdate = useCallback(() => {
    if (gameId) {
      apiCaller.get(MH_GAMES_API.STATE(gameId) + "?mode=user").then((res) => {
        const data = res.data;
  ...
  const { isLinked, linkedRound, linkedNext, linkedEnd, linkedDeleteLastRound } = useLinkedGame("mh", gameId, results.length, handleLinkedUpdate);
  ...
  const startGame = useCallback(async (refSeq) => {
    try {
      ...
      let res;
      if (useLinked) {
        res = await apiCaller.post(LINKED_GAMES_API.START, { game_type: "mh" });
      } else {
        const params = new URLSearchParams({ mode: "user" });
        if (refSeq) params.set("ref_game_seq", refSeq);
        res = await apiCaller.post(MH_GAMES_API.START + "?" + params.toString());
      }
      const data = res.data;
  ...
  const handleInput = async (inputValue) => {
    ...
    try {
      const res = isLinked
        ? await apiCaller.post(LINKED_GAMES_API.ROUND, { game_type: "mh", game_id: gameId, actual: inputValue })
        : await apiCaller.post(MH_GAMES_API.ROUND, { game_id: gameId, actual: inputValue });
      const data = res.data;
  ...
  const handleDeleteOne = useCallback(async () => {
    ...
    try {
      const res = isLinked
        ? await apiCaller.delete(LINKED_GAMES_API.LAST_ROUND, { params: { game_type: "mh", game_id: gameId } })
        : await apiCaller.delete(MH_GAMES_API.LAST_ROUND(gameId));
      const data = res.data;
  ...
  const handleNextGame = async () => {
    ...
    try {
      const res = isLinked
        ? await apiCaller.post(LINKED_GAMES_API.NEXT, { game_type: "mh", game_id: gameId })
        : await apiCaller.post(MH_GAMES_API.NEXT, null, { params: { game_id: gameId } });
