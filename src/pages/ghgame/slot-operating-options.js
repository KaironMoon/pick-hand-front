...
export const ghBetStopReasonLabel = (roundState) => {
  ...
  if (roundState?.profit_stop?.stopped) {
    ...
    return `PNL ${formatConditionNumber(profitStop.trigger_pnl)} P 상태에서 ${Math.max(0, Number(profitStop.stopped_at_round || 0))}회차 배팅액 ${formatConditionNumber(profitStop.trigger_bet_amount)} P가 종료 기준 ${formatConditionNumber(profitStop.bet_limit)} P 이상에 도달 (수익보호)`;
  }
  return null;
};
