...
import { GH_SETTINGS_API } from "@/constants/api-url";
...
export default function GhSetupPage() {
  ...
  useEffect(() => {
    Promise.all([
      apiCaller.get(GH_SETTINGS_API.BASE),
      apiCaller.get(GH_SETTINGS_API.GAME_STATUS),
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await apiCaller.put(GH_SETTINGS_API.BASE, { config });
  ...
  const handlePauseResume = async () => {
    try {
      const url = isPaused ? GH_SETTINGS_API.GAME_RESUME : GH_SETTINGS_API.GAME_PAUSE;
  ...
  const update = (key, val) => {
    setConfig((prev) => ({ ...prev, globalhit: { ...prev.globalhit, [key]: val } }));
    setDirty(true);
  };
  ...
  const ghBetType = (type) => { if (DISABLED_BET_TYPES.includes(type)) return; update("bet_type", type); };
  ...
  return (
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2 }}>
        <Box onClick={() => { const gid = searchParams.get("gameId"); navigate(gid ? `/ghgame?gameId=${gid}` : "/ghgame"); }}
