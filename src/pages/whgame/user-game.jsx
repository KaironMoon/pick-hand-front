...
import { WH_GAMES_API, LINKED_GAMES_API, USER_BET_SETTINGS_API } from "@/constants/api-url";
...
export default function WhUserGamePage() {
  ...
  const handleLinkedUpdate = useCallback(() => {
    if (gameId) {
      apiCaller.get(WH_GAMES_API.STATE(gameId) + "?mode=user").then((res) => {
        const data = res.data;
        ...
        if (data.wh_grids) setWhGrids(data.wh_grids);
  ...
  const { isLinked, linkedRound, linkedNext, linkedEnd, linkedDeleteLastRound } = useLinkedGame("wh", gameId, results.length, handleLinkedUpdate);
  ...
  const startGame = useCallback(async (refSeq) => {
    try {
      ...
      let res;
      if (useLinked) {
        res = await apiCaller.post(LINKED_GAMES_API.START, { game_type: "wh" });
      } else {
        const params = new URLSearchParams({ mode: "user" });
        if (refSeq) params.set("ref_game_seq", refSeq);
        res = await apiCaller.post(WH_GAMES_API.START + "?" + params.toString());
      }
      const data = res.data;
  ...
  const restoreGame = async (gid) => {
    try {
      const res = await apiCaller.get(WH_GAMES_API.STATE(gid) + "?mode=user");
      const data = res.data;
      setGameId(data.game_id);
      setResults(data.results || []);
      setCumPnL(data.cum_pnl || { nc: 0, gh: 0, user_a: 0, user_z: 0 });
  ...
  useEffect(() => {
    ...
    if (isNew) {
      ...
      let cancelled = false;
      apiCaller.get(WH_GAMES_API.LAST_ACTIVE + "?mode=user").then(async (res) => {
        if (cancelled) return;
  ...
  const handleInput = async (inputValue) => {
    ...
    try {
      const res = isLinked
        ? await apiCaller.post(LINKED_GAMES_API.ROUND, { game_type: "wh", game_id: gameId, actual: inputValue })
        : await apiCaller.post(WH_GAMES_API.ROUND, { game_id: gameId, actual: inputValue });
      const data = res.data;
      ...
      setCumPnL({ nc: data.cum_pnl.wh, gh: data.cum_pnl.gh, user_a: data.cum_pnl.user_a || 0, user_z: data.cum_pnl.user_z || 0 });
      setPickResult({ method: data.method, pick: data.pick, prev_picks: data.prev_picks, nickname: data.nickname, code1: data.code1, code2: data.code2 });
      if (data.wh_grids) setWhGrids(data.wh_grids);
      setGlobalhitData(data.globalhit || []);
      setGhActiveSteps(data.gh_active_steps || {});
  ...
  const handleEndingMode = async () => {
    ...
    const whStep = betData.winnerhit?.step ?? betData.wh_step ?? 1;
    const ghSteps = ghActiveSteps || {};
    const snapshot = { nc: false, gh: new Set(), pinch: new Set() };

    if (whStep > 1) snapshot.nc = true;
  ...
  const checkEndingComplete = (data) => {
    ...
    // 어드민: bet.winnerhit 사용, 유저: bet.wh_step 사용
    const whStep = data.bet?.winnerhit?.step ?? data.bet?.wh_step ?? 1;
    if (endingSnapshot.nc && whStep > 1) return false;
  ...
  const handleNextGame = async () => {
    ...
    try {
      const res = isLinked
        ? await apiCaller.post(LINKED_GAMES_API.NEXT, { game_type: "wh", game_id: gameId })
        : await apiCaller.post(WH_GAMES_API.NEXT, null, { params: { game_id: gameId } });
  ...
  return (
    <Box sx={{ p: isMobile ? 0.5 : 2 }}>
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: isMobile ? 0.4 : 0.5, mb: 1, flexWrap: "wrap" }}>
        ...
        <Box sx={{ width: isMobile ? 8 : 12 }} />
        {/* 셋업 (유저 설정) */}
        <Box onClick={() => navigate(`/whgame/user-setup${gameId ? `?gameId=${gameId}` : ""}`)} sx={{ ...controlBtnSx, cursor: "pointer", border: "2px solid #ff9800" }}>
