...
function configWithAssistValues() {
  ...
  return {
    untouched: { value: 17 },
    JMH1: { step_max: 16, miss_threshold: 7, pasi },
...
test("GH assist export contains every setup line and stages 2 through 20", () => {
  ...
  assert.match(tsv, /JMH1\tJM Helper Pick 1\tJMH1\t회차어시\t육전/);
...
test("GH assist export can be imported without changing highest steps or unrelated settings", () => {
  ...
  assert.equal(result.config.JMH1.pasi[0].assist1, "BF6");
...
test("JMH tables import scalar H/Q settings independently", () => {
  const current = configWithAssistValues();
  let edited = serializeGhAssistTsv(current);
  edited = editTsvCell(edited, "JMH1", "JMH1", "회차어시", 2, "고정P");
  edited = editTsvCell(edited, "JMH50", "JMH50", "쿼터어시", 2, "고정B");
  const result = parseGhAssistTsv(edited, current);
  assert.deepEqual(result.errors, []);
  assert.equal(result.config.JMH1.pasi[0].assist1, "고정P");
  assert.equal(result.config.JMH50.pasi[0].assist2, "고정B");
  assert.equal(result.config.JMH1.pasi[0].assist2, "해당진행");
});
